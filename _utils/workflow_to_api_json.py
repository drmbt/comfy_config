...
def check_dependencies():
    """
    Check if required dependencies are installed and provide clear installation instructions.
    If --skip-prompt is provided, attempts to install dependencies automatically.
    ...
    try:
        ...
        try:
            ...
            if pkg_resources.parse_version(playwright_version) < pkg_resources.parse_version('1.41.0'):
                if skip_prompt:
                    logger.info("Upgrading Playwright...")
                    subprocess.run([sys.executable, '-m', 'pip', 'install', '--upgrade', 'playwright'], check=True)
                else:
                    logger.error(f"Playwright version {playwright_version} is too old. Version 1.41.0 or higher is required.")
                    logger.error("Please upgrade with: pip install --upgrade playwright")
                    return False
        except pkg_resources.DistributionNotFound:
            if skip_prompt:
                logger.info("Installing Playwright...")
                subprocess.run([sys.executable, '-m', 'pip', 'install', 'playwright>=1.41.0'], check=True)
            else:
                logger.error("Playwright not found. Please install with:")
                logger.error("  pip install 'playwright>=1.41.0'")
                logger.error("Then install browsers with:")
        ...
        import subprocess
        try:
            # Use playwright --version to check installation
            ...
            if result.returncode != 0:
                if skip_prompt:
                    ...
                    logger.error("Playwright browsers not installed. Please install with:")
                    logger.error("  playwright install")
                    return False
            ...
            if skip_prompt:
                ...
                logger.error("Playwright CLI not found. Please install browsers with:")
                logger.error("  playwright install")
                return False
